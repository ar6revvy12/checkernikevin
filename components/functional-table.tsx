...
const statusConfig: Record<FunctionalStatus, { bg: string; text: string; dot: string; label: string }> = {
  running: { bg: "bg-green-500/10", text: "text-green-500", dot: "bg-green-500", label: "Running" },
  "not-running": { bg: "bg-gray-500/10", text: "text-gray-500", dot: "bg-gray-500", label: "Not Running" },
}

const statusOptions: FunctionalStatus[] = ["running", "not-running"]
...
function StatusDropdown({ value, onChange }: { value: FunctionalStatus; onChange: (status: FunctionalStatus) => void }) {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {isOpen && (
        <div className="absolute right-0 top-full mt-1 z-50 min-w-[120px] bg-white dark:bg-slate-800 rounded-lg border border-gray-200 dark:border-slate-700 shadow-lg overflow-hidden">
...
export function FunctionalTable({ tests, onUpdateStatus, onDeleteTest, onEditTest, onFiltersChange }: FunctionalTableProps) {
  const [statusFilter, setStatusFilter] = useState("all")
  const [moduleFilter, setModuleFilter] = useState("all")
  const [searchQuery, setSearchQuery] = useState("")
  const [showFilters, setShowFilters] = useState(false)

  // Get unique modules from tests
  const modules = [...new Set(tests.map((t) => t.module))].filter(Boolean)

  useEffect(() => {
    onFiltersChange?.({ status: statusFilter, module: moduleFilter, search: searchQuery })
  }, [statusFilter, moduleFilter, searchQuery, onFiltersChange])
  ...
  const hasActiveFilters = statusFilter !== "all" || moduleFilter !== "all" || searchQuery !== ""
        ...
        >
          ...
          {hasActiveFilters && (
            <span className="w-5 h-5 rounded-full bg-blue-500 text-white text-xs flex items-center justify-center">
              {[statusFilter !== "all", moduleFilter !== "all", searchQuery !== ""].filter(Boolean).length}
            </span>
          )}
        </button>
      </div>
              ...
              <tr>
                <td colSpan={9} className="px-4 py-12 text-center">
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <p className="text-gray-500 dark:text-gray-400">No functional tests found</p>
                    {hasActiveFilters && (
                      <button
                        onClick={clearFilters}
                ...
                return (
                  <tr key={test.id} className="hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors">
                    <td className="px-4 py-3">
                      <span className="text-sm font-mono text-gray-900 dark:text-white">{test.testCaseId}</span>
                    </td>
                    <td className="px-4 py-3">
                      <span className="text-sm text-gray-600 dark:text-gray-300">{test.module}</span>
                    </td>
                    <td className="px-4 py-3">
                      <ExpandableText text={test.testScenario} maxLength={60} />
                    </td>
                    <td className="px-4 py-3">
                      <ExpandableText text={test.precondition || "-"} maxLength={60} />
                    </td>
                    <td className="px-4 py-3">
                      <ExpandableText text={test.testSteps} maxLength={60} />
                    </td>
                    <td className="px-4 py-3">
