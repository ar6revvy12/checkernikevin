...
import type { RegressionPriority, RegressionStatus } from "@/types/regression"

interface AddRegressionModalProps {
  isOpen: boolean
  onClose: () => void
  onSubmit: (data: {
...
export function AddRegressionModal({ isOpen, onClose, onSubmit }: AddRegressionModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    if (!testCaseDescription.trim() || !expectedResult.trim() || !actualResult.trim()) return
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white dark:bg-slate-800 rounded-xl shadow-xl w-full max-w-4xl mx-4 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-slate-700">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Add Regression Test</h2>
          <button
            onClick={onClose}
